...
public class ForestBerryItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (state.is(ModRegistries.FOREST_HEARTWOOD_LEAVES.get())) {
            ...
            if (belowState.isAir() || belowState.canBeReplaced()) {
                ...
                if (ModRegistries.FOREST_BERRY_BUSH.get().defaultBlockState()
                        .canSurvive(level, belowPos)) {
                    // Plant the bush
                    if (!level.isClientSide()) {
                        level.setBlock(belowPos, ModRegistries.FOREST_BERRY_BUSH.get().defaultBlockState(), 3);
