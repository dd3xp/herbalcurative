...
public class MultiblockShapes {
    ...
    public VoxelShape get(Direction facing, int[] offset, boolean mirrored) {
        ...
        if (dx < 0 || dx >= xSize || dy < 0 || dy >= northShapes.length / (xSize * zSize) || dz < 0 || dz >= zSize) {
            return Shapes.block();
        }
    ...
    public static MultiblockShapes load(String resourcePath) {
        // Strip leading '/' for ClassLoader.getResourceAsStream (it doesn't use leading slash)
        String clPath = resourcePath.startsWith("/") ? resourcePath.substring(1) : resourcePath;

        InputStream is = null;
        try {
            // Try multiple classloaders — NeoForge's module system may restrict Class.getResourceAsStream
            ...
            if (is == null) {
                ...
                // that yields Shapes.block() for every position.
    ...
    private static MultiblockShapes fromJson(JsonObject root) {
        ...
        // Determine axis ranges from position keys
        int dxMin = Integer.MAX_VALUE, dxMax = Integer.MIN_VALUE;
        int dyMin = Integer.MAX_VALUE, dyMax = Integer.MIN_VALUE;
        int dzMin = Integer.MAX_VALUE, dzMax = Integer.MIN_VALUE;
